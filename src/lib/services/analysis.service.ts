...
export class AnalysisService {
  ...
  async getAnalyses(userId: string, params: PaginationParams = {}): Promise<PaginatedResponse<AnalysisDTO>> {
    const { page = 1, limit = 10, analysis_type_id } = params;
  ...
  async deleteAnalysis(id: number, userId: string): Promise<void> {
    ...
    // Delete analysis
    const { error } = await this.supabase.from("analysis").delete().eq("id", id).eq("user_id", userId);
  ...
  async logError(error: Error, userId: string, input?: Json): Promise<void> {
