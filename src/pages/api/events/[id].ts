...
type Fixture = z.infer<typeof ApiFootballFixtureSchema>;
...
export const GET: APIRoute = async ({ params, request }) => {
  ...
  if (!parse.success) {
    return new Response(JSON.stringify({ error: "Invalid params", details: parse.error.flatten() }), { status: 400 });
  }

  const sport = (parse.data.sport ?? "football").toLowerCase();
  if (sport !== "football") {
    return new Response(JSON.stringify({ error: "Unsupported sport for event-by-id", sport }), { status: 400 });
  }

  const apiKey = (import.meta as unknown as { env: { API_SPORTS_KEY?: string } }).env.API_SPORTS_KEY;
  if (!apiKey) {
    return new Response(JSON.stringify({ error: "Server misconfiguration: API_SPORTS_KEY missing" }), { status: 500 });
  }
  ...
  try {
    ...
    const resp = await apiSportsFetch("football", "fixtures", qp, apiKey, { method: "GET" }, { ttlMs: 0 });
    ...
    if (!parsed.success) {
      return new Response(JSON.stringify({ error: "Invalid upstream shape", details: parsed.error.flatten() }), {
        status: 502,
      });
    }

    let first: Fixture | undefined = parsed.data.response[0];
    ...
    if (!first && (parse.data.league || parse.data.season)) {
      try {
        ...
        if (listResp.ok) {
          ...
          const arr = (listJson?.data ?? []) as ListedEvent[];
          const found = arr.find((e) => String(e.id) === String(parse.data.id));
          if (found) {
            // Zmapuj do takiego samego kształtu jak first
            first = {
              fixture: { id: Number(found.id), date: found.startTime },
              league: { name: found.league, country: found.country ?? undefined },
        ...
        // ignore: listing fallback failed; we will return null below
      }
    }
    ...
    return new Response(JSON.stringify({ error: "Failed to fetch event", message: String(err) }), { status: 503 });
  }
};
